...
public class ConfigScreenProvider<T extends ConfigData> implements Supplier<Screen> {
    ...
    public Screen get() {
        ...
        ConfigBuilder builder = ConfigBuilder.create().setParentScreen(parent).setTitle(new TranslationTextComponent(String.format("%s.title", i13n))).setSavingRunnable(manager::save);
        ...
        if (configClass.isAnnotationPresent(Config.Gui.Background.class)) {
            ...
            ResourceLocation bgId = ResourceLocation.tryCreate(bg);
